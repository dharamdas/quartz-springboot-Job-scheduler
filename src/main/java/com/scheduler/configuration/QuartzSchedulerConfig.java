...
import com.scheduler.configuration.trigger.CustomCronTriggerFactoryBean;
import com.scheduler.quartz.job.SendEmail;
import com.scheduler.quartz.job.TestJob;
...
public class QuartzSchedulerConfig {
    ...
    public Trigger sampleJobTrigger() {
     ...
     public JobDetail sendEmail(){

        JobDetailFactoryBean factoryBean = new JobDetailFactoryBean();
        factoryBean.setJobClass(SendEmail.class);
        factoryBean.setDurability(true);
        factoryBean.setApplicationContext(context);
        factoryBean.setName("SendEmail-Job");
        factoryBean.afterPropertiesSet();
        return factoryBean.getObject();
    }
    ...
    public Trigger sendEmailTrigger()
    {
        ...
        CustomCronTriggerFactoryBean factoryBean = new CustomCronTriggerFactoryBean();
        factoryBean.setJobDetail(sendEmail());
        factoryBean.setName("SendEmail-Job-Trigger");
        factoryBean.setStartTime(LocalDateTime.now().toDate());
        factoryBean.setEndTime(LocalDateTime.now().plusHours(1).toDate());
        ...
        CronTrigger cronTrigger = factoryBean.getObject();
        return cronTrigger;
    }
